...
public class StatisticsProjection extends javax.swing.JFrame {
    ...
    private ChartType chartType = ChartType.CUMULATIVE;
    ...
    /**
     * Creates new form StatisticsProjection
     */
    ...
    public StatisticsProjection() {
        this("Sample Habit", false, "");
    }

    private void setupUI() {
        setTitle("Statistics - " + habitName);
        setDefaultCloseOperation(DISPOSE_ON_CLOSE);
        setLocationRelativeTo(null);
    ...
    private void formComponentResized(java.awt.event.ComponentEvent evt) {
        ...
        // Setup labels
        jLabel1.setText("Habit Progress: " + habitName);
        ...
        // Setup chart panel to fill the space
        chartPanel.setBackground(Color.WHITE);
        chartPanel.setBorder(BorderFactory.createLineBorder(Color.BLACK, 2));
        chartPanel.setLayout(new BorderLayout());

        // Add chart type toggle (reuse existing components if needed)
    ...
    /**
     * Load scheduled days for the habit (if it's a weekly habit)
     */
    private void loadScheduledDays() {
        String filePath = System.getProperty("user.home") + File.separator + "MyHabitTracker_Data.xlsx";
        File file = new File(filePath);
    ...
    private LocalDate parseDateFromHeader(String dateStr, LocalDate today) {
        ...
        try {
            ...
            try {
                ...
                // If the date is in the future, use previous year
            logger.log(java.util.logging.Level.SEVERE, "Error loading data from Excel", e);
        }
    }
    ...
    private LocalDate parseDateFromHeader(String dateStr, LocalDate today) {
        try {
            ...
            if (parts.length == 2) {
                ...
                LocalDate date = LocalDate.parse(parts[0] + " " + parts[1] + " " + currentYear,
    ...
    private void updateStatistics() {
        ...
        for (Map.Entry<LocalDate, Double> entry : dataPoints.entrySet()) {
            if (entry.getKey().isAfter(cutoffDate) || entry.getKey().isEqual(cutoffDate)) {
                filteredData.put(entry.getKey(), entry.getValue());
            }
        }

        if (filteredData.isEmpty()) {
            streakLabel.setText("Streak: No data");
        ...
        int streak = calculateStreak(filteredData);
        double successRate = calculateFlexibleSuccessRate(filteredData);
        int totalCompleted = (int) filteredData.values().stream()
                .filter(v -> v > 0).count();
        int totalScheduled = countScheduledDays(filteredData.keySet());
    ...
    private double calculateFlexibleSuccessRate(Map<LocalDate, Double> data) {
        ...
        int totalScheduledDays = 0;
            ...
            // For Yes/No habits, show flexible success rate
            if (scheduledDays.isEmpty()) {
                // Daily habit - show simple completion
                int totalDays = filteredData.size();
                double percentage = (totalCompleted / (double) totalDays) * 100;
                averageLabel.setText(String.format("Completion: %.1f%%", percentage));
                totalLabel.setText(String.format("Completed: %d/%d days", totalCompleted, totalDays));
            } else {
                // Weekly habit - show flexible success rate
                averageLabel.setText(String.format("Success Rate: %.1f%%", successRate));
                totalLabel.setText(String.format("Completed: %d/%d scheduled days", totalCompleted, totalScheduled));
            }
        }

        // Update chart
        chartPanel.removeAll();
        chartPanel.add(new FlexibleLineChartPanel(filteredData), BorderLayout.CENTER);
        chartPanel.revalidate();
        chartPanel.repaint();
    }

    /**
     * Calculate flexible success rate using the formula: Success Rate =
     * (Successful Days / Scheduled Days) × 100
     */
    private double calculateFlexibleSuccessRate(Map<LocalDate, Double> data) {
        if (scheduledDays.isEmpty()) {
            // Daily habit - simple percentage
            long successfulDays = data.values().stream().filter(v -> v > 0).count();
            return (successfulDays / (double) data.size()) * 100.0;
        }

        // Weekly habit - count only scheduled days
        int successfulScheduledDays = 0;
        int totalScheduledDays = 0;
    ...
    /**
     * Custom panel for drawing the flexible line chart with cumulative success
     * rate
     */
    private class FlexibleLineChartPanel extends JPanel {
        ...
        private void drawMeasurableChart(Graphics2D g2, int width, int height) {
            ...
            drawAxes(g2, width, height);
            drawYAxisLabels(g2, height, minValue, maxValue, false);

            List<LocalDate> dates = new ArrayList<>(data.keySet());
            List<Point2D> points = calculatePoints(dates, xScale, yScale, height, minValue, false);
        ...
        private void drawSuccessRateChart(Graphics2D g2, int width, int height) {
            // Calculate cumulative success rate for each day
            Map<LocalDate, Double> cumulativeRates = new TreeMap<>();
            int cumulativeSuccesses = 0;
            int cumulativeScheduled = 0;
            ...
            drawAxes(g2, width, height);
            drawYAxisLabels(g2, height, minValue, maxValue, true);

            // Draw goal line at 80%
            drawGoalLine(g2, width, height, 80.0, minValue, maxValue);
            ...
            // Draw completion markers on scheduled days
            drawScheduledDayMarkers(g2, dates, points);
        ...
        private List<Point2D> calculatePoints(List<LocalDate> dates, double xScale, double yScale,
                int height, double minValue, boolean useRates) {
            List<Point2D> points = new ArrayList<>();
        ...
        private void drawYAxisLabels(Graphics2D g2, int height, double minValue, double maxValue, boolean showPercent) {
            g2.setColor(GRID_COLOR);
            g2.setStroke(new BasicStroke(1));
            int numYDivisions = 5;
                ...
                .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanel1Layout.createSequentialGroup()
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                            ...
                            .addComponent(averageLabel, javax.swing.GroupLayout.PREFERRED_SIZE, 224, javax.swing.GroupLayout.PREFERRED_SIZE)
                            .addComponent(streakLabel, javax.swing.GroupLayout.PREFERRED_SIZE, 273, javax.swing.GroupLayout.PREFERRED_SIZE))
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                        .addGroup(jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                            .addComponent(jButton1, javax.swing.GroupLayout.DEFAULT_SIZE, javax.swing.GroupLayout.DEFAULT_SIZE, Short.MAX_VALUE)
                            .addComponent(monthlyButton, javax.swing.GroupLayout.DEFAULT_SIZE, 151, Short.MAX_VALUE))
                        .addGap(24, 24, 24))))
        );
        jPanel1Layout.setVerticalGroup(
            jPanel1Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
    ...
    private javax.swing.JPanel jPanel2;
      ...
      public static void main(String args[]) {
         ...
         * For details see http://download.oracle.com/javase/tutorial/uiswing/lookandfeel/plaf.html 
          ...
          java.awt.EventQueue.invokeLater(() -> new StatisticsProjection().setVisible(true));
      }
